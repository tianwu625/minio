...
func (ofs *OPFSObjects) encodePartFile(partNumber int, etag string, actualSize int64) string {
	return fmt.Sprintf("%.5d.%s.%d", partNumber, etag, actualSize)
}

// Returns partNumber and etag
func (ofs *OPFSObjects) decodePartFile(name string) (partNumber int, etag string, actualSize int64, err error) {
...
func (ofs *OPFSObjects) backgroundAppend(ctx context.Context, bucket, object, uploadID string) {
	ofs.appendFileMapMu.Lock()
	logger.GetReqInfo(ctx).AppendTags("uploadID", uploadID)
	file := ofs.appendFileMap[uploadID]
	if file == nil {
		file = &fsAppendFile{
			filePath: pathJoin(ofs.fsPath, minioMetaTmpBucket, ofs.fsUUID, bgAppendsDirName, fmt.Sprintf("%s.%s", uploadID, mustGetUUID())),
		}
		ofs.appendFileMap[uploadID] = file
	}
	...
	for _, entry := range entries {
		if entry == ofs.metaJSONFile {
			continue
		}
		partNumber, etag, actualSize, err := ofs.decodePartFile(entry)
		if err != nil {
...
func (ofs *OPFSObjects) ListMultipartUploads(ctx context.Context, bucket, object, keyMarker, uploadIDMarker, delimiter string, maxUploads int) (result ListMultipartsInfo, e error) {
	if err := checkListMultipartArgs(ctx, bucket, object, keyMarker, uploadIDMarker, delimiter, ofs); err != nil {
...
func (ofs *OPFSObjects) NewMultipartUpload(ctx context.Context, bucket, object string, opts ObjectOptions) (string, error) {
	if err := checkNewMultipartArgs(ctx, bucket, object, ofs); err != nil {
	...
	if err = opfsWriteFile(pathJoin(uploadIDDir, ofs.metaJSONFile), fsMetaBytes, 0o666); err != nil {
		logger.LogIf(ctx, err)
		return "", err
	}
	var ge []grantEncode
	...
	if err = opfsWriteFile(pathJoin(uploadIDDir, aclXMLFile), aclXMLBytes, 0o666); err != nil {
		logger.LogIf(ctx, err)
		return "", err
	}
...
func (ofs *OPFSObjects) CopyObjectPart(ctx context.Context, srcBucket, srcObject, dstBucket, dstObject, uploadID string, partID int,
	...
	if err := checkNewMultipartArgs(ctx, srcBucket, srcObject, ofs); err != nil {
		return pi, toObjectErr(err)
	}

	partInfo, err := ofs.PutObjectPart(ctx, dstBucket, dstObject, uploadID, partID, srcInfo.PutObjReader, dstOpts)
	if err != nil {
...
func (ofs *OPFSObjects) PutObjectPart(ctx context.Context, bucket, object, uploadID string, partID int, r *PutObjReader, opts ObjectOptions) (pi PartInfo, e error) {
	...
	data := r.Reader
	if err := checkPutObjectPartArgs(ctx, bucket, object, ofs); err != nil {
	...
	defer opfsRemoveFile(ctx, tmpPartPath)
	...
	partPath := pathJoin(uploadIDDir, ofs.encodePartFile(partID, etag, data.ActualSize()))

	// Make sure not to create parent directories if they don't exist - the upload might have been aborted.
	if err = opfsRename(tmpPartPath, partPath); err != nil {
		if err == errFileNotFound || err == errFileAccessDenied {
			return pi, InvalidUploadID{Bucket: bucket, Object: object, UploadID: uploadID}
		}
	...
	go ofs.backgroundAppend(ctx, bucket, object, uploadID)

	fi, err := opfsStatFile(ctx, partPath)
	if err != nil {
		return pi, toObjectErr(err, minioMetaMultipartBucket, partPath)
	}
	return PartInfo{
...
// - encrypted
// - compressed
func (ofs *OPFSObjects) GetMultipartInfo(ctx context.Context, bucket, object, uploadID string, opts ObjectOptions) (MultipartInfo, error) {
	minfo := MultipartInfo{
		Bucket:   bucket,
		Object:   object,
		UploadID: uploadID,
	...
	if err := checkListPartsArgs(ctx, bucket, object, ofs); err != nil {
	...
	fsMetaBytes, err := opfsReadFile(pathJoin(uploadIDDir, ofs.metaJSONFile))
	if err != nil {
		logger.LogIf(ctx, err)
		return minfo, toObjectErr(err, bucket, object)
	}
...
func (ofs *OPFSObjects) ListObjectParts(ctx context.Context, bucket, object, uploadID string, partNumberMarker, maxParts int, opts ObjectOptions) (result ListPartsInfo, e error) {
	if err := checkListPartsArgs(ctx, bucket, object, ofs); err != nil {
		return result, toObjectErr(err)
	}
	...
	for _, entry := range entries {
		...
		partNumber, currentEtag, actualSize, derr := ofs.decodePartFile(entry)
		if derr != nil {
			// Skip part files whose name don't match expected format. These could be backend filesystem specific files.
			continue
		}

		entryStat, err := opfsStatFile(ctx, pathJoin(uploadIDDir, entry))
		if err != nil {
			continue
		}
	...
	rc, _, err := opfsOpenFile(ctx, pathJoin(uploadIDDir, ofs.metaJSONFile), 0)
	if err != nil {
		if err == errFileNotFound || err == errFileAccessDenied {
			return result, InvalidUploadID{Bucket: bucket, Object: object, UploadID: uploadID}
		}
...
func (ofs *OPFSObjects) CompleteMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, parts []CompletePart, opts ObjectOptions) (oi ObjectInfo, e error) {
	var actualSize int64

	if err := checkCompleteMultipartArgs(ctx, bucket, object, ofs); err != nil {
	...
	appendFilePath := pathJoin(ofs.fsPath, minioMetaTmpBucket, ofs.fsUUID, "bg-appends", fmt.Sprintf("%s.%s", uploadID, mustGetUUID()))
	...
	//    from the beginning
	ofs.backgroundAppend(ctx, bucket, object, uploadID)
	...
	if appendFallback {
		if file != nil {
			opfsRemoveFile(ctx, file.filePath)
		}
		for _, part := range parts {
			partFile := getPartFile(entriesTrie, part.PartNumber, part.ETag)
			if partFile == "" {
	...
	destLock := ofs.NewNSLock(bucket, object)
	...
	metaFile, err := ofs.rwPool.OpfsWrite(fsMetaPath)
	var freshFile bool
	if err != nil {
		...
		metaFile, err = ofs.rwPool.OpfsCreate(fsMetaPath)
		if err != nil {
			logger.LogIf(ctx, err)
			return oi, toObjectErr(err, bucket, object)
		}
	...
	defer func() {
		...
		if e != nil && freshFile {
			...
			opfsRemoveMeta(ctx, bucketMetaDir, fsMetaPath, tmpDir)
		}
	}()

	// Read saved fs metadata for ongoing multipart.
	fsMetaBuf, err := opfsReadFile(pathJoin(uploadIDDir, ofs.metaJSONFile))
	if err != nil {
		logger.LogIf(ctx, err)
		return oi, toObjectErr(err, bucket, object)
	}
	...
	aclXMLBuf, err := opfsReadFile(pathJoin(uploadIDDir, aclXMLFile))
	if err != nil {
		logger.LogIf(ctx, err)
		return oi, toObjectErr(err, bucket, object)
	}
	...
	{
		fsTmpObjPath := pathJoin(ofs.fsPath, minioMetaTmpBucket, ofs.fsUUID, mustGetUUID())
		defer opfsRemoveAll(ctx, fsTmpObjPath) // remove multipart temporary files in background.
...
// honored.
func (ofs *OPFSObjects) AbortMultipartUpload(ctx context.Context, bucket, object, uploadID string, opts ObjectOptions) error {
	if err := checkAbortMultipartArgs(ctx, bucket, object, ofs); err != nil {
		return err
	}
...
func (ofs *OPFSObjects) getAllUploadIDs(ctx context.Context) (result map[string]string) {
	result = make(map[string]string)

	entries, err := opfsReadDir(pathJoin(ofs.fsPath, minioMetaMultipartBucket))
	if err != nil {
		return
	}
	for _, entry := range entries {
		uploadIDs, err := opfsReadDir(pathJoin(ofs.fsPath, minioMetaMultipartBucket, entry))
		if err != nil {
			continue
		}
		...
		for i := range uploadIDs {
			...
			result[uploadID] = pathJoin(ofs.fsPath, minioMetaMultipartBucket, entry, uploadID)
		}
	}
	return
}
...
func (ofs *OPFSObjects) cleanupStaleUploads(ctx context.Context) {
	...
	for {
		...
		case <-bgAppendTmpCleaner.C:
			...
			foundUploadIDs := ofs.getAllUploadIDs(ctx)

			// Remove background append map from the memory
			ofs.appendFileMapMu.Lock()
			for uploadID := range ofs.appendFileMap {
			...
			bgAppendsDir := pathJoin(ofs.fsPath, minioMetaTmpBucket, ofs.fsUUID, bgAppendsDirName)
			entries, err := opfsReadDir(bgAppendsDir)
			if err != nil {
				break
			}
			for _, entry := range entries {
				...
				_, ok := foundUploadIDs[uploadID]
				if !ok {
					opfsRemoveFile(ctx, pathJoin(bgAppendsDir, entry))
				}
			}
		...
		case <-expiryUploadsTimer.C:
			...
			uploadIDs := ofs.getAllUploadIDs(ctx)

			for uploadID, path := range uploadIDs {
				fi, err := opfsStatDir(ctx, path)
				if err != nil {
					continue
				}
				if now.Sub(fi.ModTime()) > expiry {
					opfsRemoveAll(ctx, path)
					// Remove upload ID parent directory if empty
					opfsRemoveDir(ctx, filepath.Base(path))

					// Remove uploadID from the append file map and its corresponding temporary file
					ofs.appendFileMapMu.Lock()
					bgAppend, ok := ofs.appendFileMap[uploadID]
					if ok {
						_ = opfsRemoveFile(ctx, bgAppend.filePath)
						delete(ofs.appendFileMap, uploadID)
					}
					ofs.appendFileMapMu.Unlock()
				}
